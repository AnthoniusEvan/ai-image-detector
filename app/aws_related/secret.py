...
SM_SECRET_APP_NAME = os.getenv("SM_SECRET_APP_NAME")  # e.g. n11671025-aipic-jwt
...
def get_jwt_secret() -> str:
    """
    Returns the JWT secret from Secrets Manager.
    Accepts either:
      - plain SecretString (recommended for the tutorial), or
      - JSON SecretString with {"jwt_secret": "..."}.
    """
    ...
    try:
        ...
        if not secret_str:
            # Binary fallback (unlikely here)
            secret_str = resp.get("SecretBinary", b"").decode("utf-8", errors="ignore")

        # Try JSON first; if not JSON, treat as raw string.
        try:
