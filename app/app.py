...
app = FastAPI(title="AI Image Detector", description="API for detecting if an image is real or AI-generated ", version="0.0.1")
...
def generate_access_token(id, username):
    payload = {'id': id, 'username': username, 'exp': datetime.datetime.now(timezone.utc) + datetime.timedelta(minutes=30)}
...
async def result_page(image_id: str, request: Request, user=Depends(browser_auth)):
    ...
    if not presigned_url:
        raise HTTPException(status_code=404, detail="Image not found")
    return templates.TemplateResponse("result.html", {"request": request, "image_id": image_id, "image_url": presigned_url, "prediction": image_data['prediction'], "confidence": image_data['confidence'], "user_prediction": image_data.get('user_prediction')})
...
async def admin_uploads(user=Depends(browser_auth), limit: int = Query(10, ge=1, le=100), offset: int = Query(0, ge=0), sort_by: str = Query("uploaded_at"), order: str = Query("desc", regex="^(asc|desc)$"), username: str | None = None, prediction: str | None = None):
    ...
    for img in images:
        ...
        uid = img.get('user_id')
        img['username'] = get_username(uid) if uid else None
    return images
